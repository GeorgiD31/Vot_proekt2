...
app.post('/posts', async (req, res) => {
  const { time } = req.body; 
  console.log('Click Count:', time);
  try {
    const conn = await pool.getConnection();
    const result = await conn.query('INSERT INTO posts (time) VALUES (?)', [time]);
    res.json({ id: result.insertId, time });
  } catch (err) {
    console.error('Click Count saving failed:', err);
    res.status(500).json({ error: err.message });
  }
});
